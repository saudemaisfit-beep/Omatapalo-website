          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 'clamp(16px,2.5vw,32px)', alignItems: 'start' }} className="media-grid">
            ...
            {featured && (
              <a
                ...
                style={{ display: 'block', opacity: 0, overflow: 'hidden', borderRadius: 4, position: 'relative', aspectRatio: '4/3', background: '#0d1622', textDecoration: 'none', cursor: 'pointer' }}
              >
                {featured.cover_image && (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={featured.cover_image} alt={featured.title} style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(12px,2vw,20px)' }}>
              {side.map((n, i) => (
                <a
                  ...
                  onMouseLeave={() => setHoveredCard(null)}
                  style={{ display: 'flex', gap: 'clamp(14px,1.5vw,20px)', opacity: 0, alignItems: 'stretch', background: '#fff', borderRadius: 4, overflow: 'hidden', textDecoration: 'none', border: `1.5px solid ${hoveredCard === i + 1 ? '#1a396e' : '#E8EDF5'}`, transition: 'border-color .25s, box-shadow .3s', boxShadow: hoveredCard === i + 1 ? '0 12px 36px rgba(26,57,110,0.12)' : 'none' }}
                >
                  <div style={{ position: 'relative', width: 'clamp(100px,30%,160px)', flexShrink: 0, overflow: 'hidden' }}>
                    {n.cover_image && (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={n.cover_image} alt={n.title} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', transition: 'transform .6s ease', transform: hoveredCard === i + 1 ? 'scale(1.08)' : 'scale(1)' }} />
                    )}
                  </div>
